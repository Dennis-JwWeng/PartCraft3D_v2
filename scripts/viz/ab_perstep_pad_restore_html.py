#!/usr/bin/env python3
"""512 perstep A/B — pad2 vs pad2_restore (white-model S2-shape), embedded HTML.

For each (obj, edit) shows: condition input/edited + BEFORE 5 views + the
white-model S2-shape AFTER 5 views for each experiment tree.  All trees use the
SAME recipe (masked S1 + perstep S2 + edit_res=512 + force_white_model); the only
variable is pad / restore — so geometry differences isolate that.

Trees compared (any present under data/Pxform_v2 are included, in this order):
  _exp_masked_perstep_r512_pad0          (control)
  _exp_masked_perstep_r512_pad2          (dilation)
  _exp_masked_perstep_r512_pad2_restore  (dilation + occupancy restore)

    python scripts/viz/ab_perstep_pad_restore_html.py
"""
...
TREES = [
    ("pad0 (control)",        "_exp_masked_perstep_r512_pad0",         "#8a8f98"),
    ("pad2",                  "_exp_masked_perstep_r512_pad2",         "#7fd1ff"),
    ("pad2 + restore",        "_exp_masked_perstep_r512_pad2_restore", "#ffb454"),
]
SHARD = "08"
...
def main() -> None:
    ...
    # work list = union of (obj, edit) that completed in ANY tree
    keys = set()
    ...
    blocks = []
    for obj, eid in keys:
        # condition + before come from the first tree that has them
    ...
    out = DATA / "_scratch" / "ab_compare" / "perstep_pad_restore_r512.html"
    out.parent.mkdir(parents=True, exist_ok=True)
    html = f"""<!doctype html><html><head><meta charset="utf-8">
<title>512 perstep pad/restore A/B</title><style>
...
<h1>512 perstep S2-shape — pad/restore A/B</h1>
<div class="note">同一配方(masked S1 + perstep S2 + edit_res=512 + 白模),唯一变量 = pad/restore。{legend}。{len(keys)} edits。</div>
{''.join(blocks)}
</body></html>"""
    out.write_text(html)
