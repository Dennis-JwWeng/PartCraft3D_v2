...
#   GPUS=3,4,5,6,7 SHARD=01 bash scripts/cleaning/run_vlm_cleaning_multi_gpu.sh
#
#   # Deletion-only (no TRELLIS, fast):
#   ONLY_TYPES=deletion GPUS=3 SHARD=01 bash scripts/cleaning/run_vlm_cleaning_multi_gpu.sh
...
for i in "${!GPU_ARRAY[@]}"; do
    ...
    python scripts/cleaning/run_vlm_cleaning.py \
        --root "$ROOT" \
