...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def encode_image(self, image: Union[torch.Tensor, list[Image.Image]]) -> torch.Tensor:
        ...
        output = self.models['image_cond_model'](image, is_training=True)
    ...
    def run(
        ...
        slat = self.sample_slat(cond, coords, None, slat_sampler_params)
        # torch.save(slat.feats, f"outputs/slat/{name}_ori_feats.pt")
        # torch.save(slat.coords, f"outputs/slat/{name}_ori_coords.pt")
        return self.decode_slat(slat, formats)

    @torch.no_grad()
    def retrieve_slat(self):
        slat = sp.SparseTensor(
            feats=torch.load("outputs/full/slat/animalcar_feats.pt"),
            coords=torch.load("outputs/full/slat/animalcar_coords.pt"),
        )
        return self.decode_slat(slat, ['gaussian'])
