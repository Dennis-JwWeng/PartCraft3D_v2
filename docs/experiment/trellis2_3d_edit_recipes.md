...
| R8 | **512 编辑分辨率** | 任意 S1 + `edit_res:512` | 任意 S2 @512 | 分辨率对照：S2 在 32³ + `_512` 模型 + decode@512。`configs/experiments/pipeline_v3_trellis2_flowedit_free_r512.yaml`(R4@512) / `_masked_posthoc_r512_pad0.yaml`(R3@512)；输出 `_exp_flowedit_free_r512` / `_exp_masked_posthoc_r512_pad0`，A/B vs 1024。 |

> R4 = 当前 `configs/experiments/pipeline_v3_trellis2_flowedit_free_r1024.yaml` 的配置。
...
MACHINE_ENV=configs/machine/local_trellis2.env \
  bash run_pipeline_v3_shard_trellis2.sh shard08_glbsmoke \
       configs/experiments/pipeline_v3_trellis2_flowedit_free_r1024.yaml
```
...
STAGES=trellis2_encode,trellis2_preview MACHINE_ENV=configs/machine/local_trellis2.env \
  bash run_pipeline_v3_shard_trellis2.sh e512_flowedit_full \
       configs/experiments/pipeline_v3_trellis2_flowedit_free_r512.yaml
