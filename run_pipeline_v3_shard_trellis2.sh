#!/usr/bin/env bash
# pipeline_v3 stage scheduler — TRELLIS.2 variant.  (repo-root entry point)
#
# Same scheduling/topology as the v1 run_pipeline_v3_shard.sh, but:
#   * dispatches `python -m partcraft.pipeline_v3.run_trellis2`
...
#   3. Invoking `python -m partcraft.pipeline_v3.run_trellis2 --stage <name>`
...
# ─── machine env ─────────────────────────────────────────────────────
ENV_FILE="${MACHINE_ENV:-configs/machine/local_trellis2.env}"
[ -f "$ENV_FILE" ] || {
    echo "[ERROR] Machine config not found: ${ENV_FILE}"
    echo "  TRELLIS.2 default: configs/machine/local_trellis2.env"
    echo "  (override via MACHINE_ENV=...)"
    exit 1
}
...
# ─── Phase-1 edit-generation mode ────────────────────────────────────
# This TRELLIS.2 shard defaults to image_semantic (Mode A): the VLM reads
# the overview 分割图 and a rule-based visibility pre-pass drops invisible
# parts before edit generation (partcraft/pipeline_v3/gen_edits_image.py).
# Override with EDIT_GEN_MODE=text to fall back to caption-only Mode B.
export EDIT_GEN_MODE="${EDIT_GEN_MODE:-image}"
...
echo "============================================================"
echo "  pipeline_v3 shard run (TRELLIS.2)"
echo "============================================================"
echo "  tag         : $TAG"
echo "  shard       : $SHARD"
echo "  config      : $CFG"
...
echo "  edit_gen    : ${EDIT_GEN_MODE}  (image=Mode A overview+visibility prefilter; text=Mode B)"
...
run_stage() {
    ...
    # FORCE=1 overrides this — the user wants a re-run regardless of status.
    local _started=0
    if [ "$STAGE_SERVERS" != "none" ]; then
        ...
        if [ "${FORCE:-0}" = "1" ]; then
            ...
            _pending=$(
                LIMIT="${LIMIT:-}" \
                "$PY_PIPE" -m partcraft.pipeline_v3.run_trellis2 \
